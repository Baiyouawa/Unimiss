...
from common.experiment_utils import ensure_cache_dir, load_main_dataset, summarize_metrics  # noqa: E402
...
def select_beijing_stations(processed: dict, station_count: int) -> dict:
    if station_count is None or station_count <= 0:
        return processed
    ...
    current_df = df[df["station"] == stations[0]]
    feature_cols = [c for c in current_df.columns if c not in drop_cols]
    per_station = len(feature_cols)
    if per_station <= 0:
        return processed

    selected = stations[:station_count]
    indices = []
    for i, station in enumerate(stations):
        if station in selected:
            start = i * per_station
            indices.extend(range(start, start + per_station))

    def slice_feat(arr):
        return arr[..., indices]

    for key in ["train_X", "val_X", "test_X", "train_X_ori", "val_X_ori", "test_X_ori"]:
        if key in processed:
            processed[key] = slice_feat(processed[key])
    processed["n_features"] = processed["train_X"].shape[-1]
    processed["selected_stations"] = selected
    return processed
...
parser.add_argument(
    ...
    choices=[
        ...
        "italy_air_quality",
        "beijing_multi_site_air_quality",
    ],
...
parser.add_argument(
    "--beijing_station_count",
    type=int,
    default=4,
    help="北京多站点仅使用前 N 个站点（<=0 表示全量）",
)
parser.add_argument(
    "--prep_n_steps",
    type=int,
    default=48,
    help="时间序列预处理窗口长度 n_steps",
)
parser.add_argument(
    "--cuda_device",
    type=int,
...
if DATASET_NAME == "electricity_transformer_temperature":
    ...
    data = preprocess_electricity_load_diagrams(rate=0.01, n_steps=prep_n_steps, pattern="point")
elif DATASET_NAME == "italy_air_quality":
    data = load_main_dataset(DATASET_NAME, prep_n_steps)
elif DATASET_NAME == "beijing_multi_site_air_quality":
    data = preprocess_beijing_air_quality(rate=0.01, n_steps=prep_n_steps, pattern="point")
    data = select_beijing_stations(data, args.beijing_station_count)
    if args.beijing_station_count and args.beijing_station_count > 0:
        print(f"Beijing stations selected: {data.get('selected_stations', [])}")
else:
...
allowed = {
    "electricity_transformer_temperature": {"mar", "mnar_t", "mnar_x", "mix"},
    "electricity_load_diagrams": {"mar", "mnar_t"},
    "italy_air_quality": {"mar", "mnar_x", "mix"},
    "beijing_multi_site_air_quality": {"mar", "mnar_x"},
}
if mask_type not in allowed[DATASET_NAME]:
    raise ValueError(f"{DATASET_NAME} 不支持缺失机制 {mask_type}，可选 {allowed[DATASET_NAME]}")
if mask_type == "mix":
    if DATASET_NAME not in mix_mnar_map:
        raise ValueError(f"{DATASET_NAME} 不支持 mix 场景")
    if missing_rate not in mix_missing_rates:
...
for seed in run_seeds:
    ...
    train_set = {"X": train_X}
    val_X_ori = data.get("val_X_ori", val_X_raw)
    val_set = {
        "X": val_X,
        "X_ori": val_X_ori,
    }
    ...
    test_X_ori = data.get("test_X_ori", test_X_raw)
    ...
    imputation = torch.as_tensor(imputation).cpu().numpy()

    # 对齐维度 [batch, time, feature]
    ...
    manual_save_path = seed_dir / "manually_saved_saits_model"
    saits.save(saving_path=str(manual_save_path), overwrite=True)
    saits.load(f"{manual_save_path}.pypots")
