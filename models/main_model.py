...
class FGTI(nn.Module):
    ...
    def calc_loss(self, observed_data, observed_dataf, cond_mask, observed_mask, side_info):
        ...
        # 将条件输入扩展为 [B, 2K, L]（观测 + 插值特征），匹配 Transformer 投影维度 enc_in*2
        cond_obs = torch.cat([cond_mask * observed_data, observed_dataf], dim=1)
    ...
    def impute(self, observed_data, observed_dataf, cond_mask, side_info, n_samples=100):
        ...
        for i in range(n_samples):
            ...
            for t in range(self.num_steps - 1, -1, -1):
                ...
                cond_obs = torch.cat([cond_mask * observed_data, observed_dataf], dim=1)
                predicted = self.diffmodel(total_input, side_info, cond_obs, torch.tensor([t]).to(self.device))
